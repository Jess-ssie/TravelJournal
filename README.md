...
**Обрано** бібліотеку `System.Text.Json`, адже вона добре інтегрується з C#, зручна для збереження `List<T>` у
 файли (`.json`). Не потребує сторонніх пакетів на зразок `Newtonsoft.Json`.
...
---
## Заключне слово
`TravelJournal` — це зручний інструмент для ведення особистого щоденника подорожей у консольному режимі. 
