...
### Відхилені альтернативні технології
...
### Схема взаємодії
1. **Користувач** вводить команду через CLI.
2. **Program.cs** приймає команду, знаходить відповідний об'єкт у словнику команд і викликає його метод `Execute(args)`.
3. **Команда** (із папки `Commands/`) виконує валідацію вхідних даних (через компоненти з `Validate/`), після чого звертається до відповідного **репозиторію** або **сервісу**.
4. **Репозиторій** (із папки `Repositories/`) працює з моделями (`Models/`) та зберігає/отримує дані у/з JSON-файлів (`Data/`).
5. **Сервіси** (із папки `Services/`) виконують допоміжну логіку, наприклад, ведення історії команд.
6. **Enums/** та **Validate/** використовуються на різних етапах для забезпечення коректності та структурованості даних.

#### Вся система взаємопов'язана:
Користувач → **Program.cs** → **Commands/** → (**Validate/**) → **Repositories/**/**Services/** → **Models/** → **Data (JSON)**
Такий підхід забезпечує гнучкість, майбутню розширюваність і логічну ізоляцію компонентів.
